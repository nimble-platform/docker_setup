...
if [[ "$1" = "infra" ]]; then
	...
	# update services
	docker-compose -f services/docker-compose-prod-efac.yml --project-name ${SERVICE_PROJECT} pull

	# start services
	docker-compose -f services/docker-compose-prod-efac.yml --project-name ${SERVICE_PROJECT} up \
		-d \
		--build \
		--force-recreate \
		identity-service business-process-service frontend-service catalogue-service \
		frontend-service-sidecar indexing-service data-aggregation-service trust-service
